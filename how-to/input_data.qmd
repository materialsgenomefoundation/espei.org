...
## Phase Descriptions {#input_phase_descriptions}
...
-   The type of quantity is expressed using the `output` key. This can
    ...
    This is tracked by `85`{.interpreted-text role="issue"} on GitHub.
...
Only activity data is supported at the moment. Support for other data
types is tracked by `104`{.interpreted-text role="issue"}.
::::
...
[\[Bocklund2019\]](#Bocklund2019){.citation}) will be undefined and no
...
[^1]: `ZPF` after the \"Zero Phase Fraction\" method
    [\[Bocklund2019\]](#Bocklund2019){.citation} used to compute the
