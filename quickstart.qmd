...
files and described in detail at the [YAML input schema](reference/writing_input.qmd) page. All of your input datasets should be validated by
...
If you have only
[non-equilibrium thermochemical data](how-to/dataset_schema.qmd#non_equilibrium_thermochemical_data), e.g. heat capacity, entropy and enthalpy data and mixing
...
files shown above. See the [YAML input schema](reference/writing_input.qmd) for a full description of all possible inputs.
...
examples are shown on the [Recipes](reference/recipes.qmd) page.
...
[`data_weights` in YAML input schema](reference/writing_input.qmd#data_weights)). The total log
...
You may be interested in the [input_mcmc_symbols](reference/writing_input.qmd#symbols) input parameter to specify which parameter subset to
optimize.
