...
Here you can find some useful snippets of code to make using ESPEI
easier.
...
expecting. This script plots a binary phase diagram.

*TIP:* Using this in Jupyter Notebooks make it really fast to update and
watch your progress.
...
![](../_static/dataplot-recipe-cu-mg.png){width="6.94444in"}
...
![](../_static/phase-diagram-recipe-cr-ni.png){width="6.94444in"}
...
predictions. The `espei.plot.plot_interaction`{.interpreted-text
role="py:func"} code will plot the predicted interaction from the
...
Running for the single sublattice LIQUID phase in Cu-Mg gives the
following output after parameter selection:

![](../_static/cu-mg-plot_parameters-liquid.png)
...
The example below is for *one* parameter. Running the snippet above will
plot all of the parameters on separate plots.
